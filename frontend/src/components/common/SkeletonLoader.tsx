...
export const TableSkeleton: React.FC<{ rows?: number }> = ({ rows = 6 }) => {
  return (
    <div className="w-full divide-y divide-slate-200 animate-pulse">
      {Array.from({ length: rows }).map((_, i) => (
        <div key={i} className="flex items-center px-4 py-3.5 gap-4">
          <div className="w-4 h-4 rounded bg-slate-200 shrink-0" />
          <div className="w-16 h-4 rounded bg-slate-200 shrink-0" />
          <div className="flex-1 space-y-2">
            <div className="w-3/4 h-4 rounded bg-slate-200" />
            <div className="w-1/3 h-3 rounded bg-slate-100" />
          </div>
          <div className="w-28 h-4 rounded bg-slate-200 shrink-0 hidden md:block" />
          <div className="w-24 h-6 rounded-full bg-slate-200 shrink-0" />
          <div className="w-20 h-5 rounded bg-slate-200 shrink-0 hidden sm:block" />
          <div className="w-28 h-4 rounded bg-slate-200 shrink-0 hidden lg:block" />
          <div className="w-16 h-3 rounded bg-slate-200 shrink-0" />
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 animate-pulse">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="p-4 rounded-xl border border-slate-200 bg-white space-y-3 shadow-xs">
